...
GlobalCountryTemp %>%  filter(Country=="Global",  year(Date) > 1899) %>% 
autoplot(LandOceanAvgTemp %>% difference(12)) 
...
```{r}
fitting <- GlobalCountryTemp %>% filter(Country == "Global", year(Date)>1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>%
  model(
        ar3 = ARIMA(diff ~ pdq(4, 1, 0) + PDQ(0,0,0)),
        ar2 = ARIMA(diff ~ pdq(3, 1, 0) + PDQ(0,0,0)),
        arma11 = ARIMA(diff ~ pdq(1, 1, 1) + PDQ(0,0,0)), 
        auto1 = ARIMA(diff),
        auto2 = ARIMA(diff ~ pdq(d=1))
        )
fitting %>% tidy()
#fitting %>% tidy() %>% filter(.model == "ar3")
fitting %>% augment()
fitting %>% select(ar3) %>% augment()
```


```{r}
GlobalCountryTemp %>% filter(Country=="Global", year(Date) > 1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>% gg_tsdisplay(diff, plot_type = 'partial')
GlobalCountryTemp %>% filter(Country=="Global", year(Date) > 1899) %>% mutate(diff = difference(LandOceanAvgTemp, 12)) %>% gg_tsdisplay(diff, plot_type = 'partial')
```

```{r}
fit <-   GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff = difference(LandOceanAvgTemp, 12)) %>%  model(arima = ARIMA(diff ~ pdq(d=1))) %>% fabletools::report()
```


```{r}
fit2 <-   GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>%  model(arima = ARIMA(diff))

fabletools::report(fit2)

fit3 <-   GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>%  model(arima = ARIMA(diff ~ pdq(4, 1, 0) + PDQ(0,0,0)))
