...
z1 <- GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff1 = difference(LandAvgTemp, 12))

z2 <- GlobalCountryTemp %>%  filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff2 = difference(LandOceanAvgTemp, 12)) 

```
The next step was to examine the sample autocorrelation and partial autocorrelation plots, and use them to try and make inference about possible arima models to fit. For both series, the sacf plot had a decaying sine wave and the pacf had a fast decay, with lag 3 being the last significant value. These both suggested that an ARMA(3,0) model be a good option to try. However, there were some spikes later on in the pacf plot around lag 12 which implied that the simple ARMA may have problems. 

```{r, warning = FALSE}
z1 %>% gg_tsdisplay(diff1, plot_type = 'partial', lag_max = 15)
z2 %>% gg_tsdisplay(diff2, plot_type = 'partial', lag_max = 15)
```
...
sarima_land <- z1 %>% model(arima = ARIMA(diff1))
arima_land <- z1 %>%  model(arima = ARIMA(diff1 ~ pdq(3, 0, 0) + PDQ(0,0,0))) 
sarima_land %>% gg_tsresiduals(lag_max=15)
augment(sarima_land) %>%
  features(.resid, ljung_box, lag = 10, dof = 3)

arima_land %>% gg_tsresiduals(lag_max=15)
augment(arima_land) %>%
  features(.resid, ljung_box, lag = 10, dof = 3)
...
```{r warning=FALSE}
sarima_land_ocean <- z2 %>%  model(arima = ARIMA(diff2)) 
arima_land_ocean <- z2 %>%  model(arima = ARIMA(diff2 ~ pdq(3, 0, 0) + PDQ(0,0,0))) 


sarima_land_ocean %>% gg_tsresiduals(lag_max=15)
augment(sarima_land_ocean) %>%
  features(.resid, ljung_box, lag = 10, dof = 3)# as there are 3 parameters


arima_land_ocean %>% gg_tsresiduals(lag_max=15)
...
sarima_land %>% forecast(h = "8 years") %>% 
  autoplot(filter(z1, year(Date)>2000))
sarima_land_ocean %>% forecast(h = "8 years") %>% 
  autoplot(filter(z2, year(Date)>2000))
```
