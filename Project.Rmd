...
GlobalCountryTemp %>%
  ...
  summarise(MeanLandAvgTempEurope = mean(LandAvgTemp)) %>%
  as_tsibble(index = Date) -> land_temp_mean_europe

  
dec_europe <- land_temp_mean_europe %>% 
  model(stl = STL(MeanLandAvgTempEurope))

components(dec_europe)
```
