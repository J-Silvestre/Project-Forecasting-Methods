...
The second to last topic in this report will focus entirely in applying Exponential Smoothing methods to the given time-series. 
Firstly, ETS forecasting will be applied to the global average temperatures, as well as land average temperatures for each continent. Secondly, an assessment of the forecast accuracy will be analyzed.



### Global Land Average Temperature

Due to the strong presence of seasonality in our data, the most appropriate Exponential Smoothing methods would most likely be the ones with a seasonal component, such as Holt-Winters Additive Method or Holt-Winters Multiplicative Method. Besides that, a more simple ETS Method without a trend component could also be considered.Therefore, in this first step, these three exponential smoothing methods are applied.
...
fit_global_land %>% 
  forecast(h = "8 years") %>% 
  autoplot(filter(global_temp, year(Date)>2000),level = NULL) + labs(title = "Global Land Average Temperature ETS Forecast", caption = "fig. 40")
```
As expected, the ETS forecasts applied to these time series had a very small AICc difference and, therefore, even though ANA was the one that performed the best, the three methods applied do not differ much from one another. Naturally, the Holt Winters multiplicative method was the one that performed the worst because seasonal variations in this case do not change proportionally to the level of the series.


```{r, warning = FALSE}
fit_ANA %>% 
  gg_tsresiduals() + labs(title = "Residual plots for Global Average Temperature ETS(A,N,A) Forecast", caption = "fig. 41")

```
By looking at the distribution plot of the residuals, it is possible to visualize that the residuals do tend to cluster around zero. Besides that, only 2 out of 24 lags in the ACF are barely significant. This means that the residuals do seem to be white noise. Which leads to the conclusion that the residuals are normal and uncorrelated. Therefore, the forecasts should also follow a normal distribution.
This indicates that using an exponential smoothing method that is comprised of an additive error with a null trend and additive seasonal component would yield fairly decent forecasts.
...
fit_continents %>% 
  components() %>% 
  autoplot() + labs(title = "ETS Components", caption = "fig. 42")
...
fit_continents %>% 
  forecast(h = "8 years") %>% 
  autoplot(land_continents) + facet_grid(Country~., scales = "free_y", shrink = T)  + labs(title = "Land Average Temperature ETS Forecast by Continent", caption = "fig. 43")
```
...
fit_continents %>% 
  augment() %>% 
  autoplot(.resid) + facet_grid(Country~., scales = "free_y", shrink = T) + labs(title = "Residuals from ANA ETS per Continent", caption = "fig. 44")

fit_continents %>% 
  augment() %>% 
  ACF(.resid) %>% 
  autoplot() + facet_grid(Country~., scales = "free_y", shrink = T) + labs(title = "Residuals from ANA ETS per Continent", caption = "fig. 44")
```
...
GlobalCountryTemp %>%  filter(Country=="Global",  year(Date) > 1899) %>% 
autoplot(LandAvgTemp)

```
We plotted the time series of interest in view of beggining the process to fit ARIMA models to the data. Both showed high seasonality and some moderate positive linear trend. 

```{r}
GlobalCountryTemp %>%  filter(Country=="Global",  year(Date) > 1899) %>% 
autoplot(LandAvgTemp %>% difference(12))


#GlobalCountryTemp %>%  filter(Country=="Global") %>% autoplot(LandAvgTemp %>% difference(12) %>% difference())
```
...
#GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff_lat = (difference(difference(LandAvgTemp, 12)))) %>%
 # features(diff_lat, unitroot_pp)
GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899)%>% mutate(diff_lat = (difference(LandAvgTemp, 12))) %>%
...
#GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899)  %>% mutate(diff_lat = (difference(difference(LandAvgTemp, 12)))) %>%
 # features(diff_lat, unitroot_kpss)
```
First we wanted to check for any necessary differencing or box-cox transformations before moving into ARIMA modelling. The variance appeared to be very stable, so we did not feel that there was any need for a log transformation. We have already mentioned that there was high seasonality in the data, which is no surprise as it is to do with weather. To handle this, we first differenced at lag 12. The positive linear trend was very weak but still existed in the data; we had to decide whether it was necessary to difference it out or not. We used some tests to help us make the decision; the pp and the kpss tests. The pp test had a p-value of 0.01, rejecting H0 of a unit root, and the kpss test had a p-value of 0.1, not rejecting H0 that there was not a unit root. The tests agreed that an extra difference for trend was not necessary. 

```{r}
GlobalCountryTemp %>% filter(Country == "Global", year(Date)>1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>% autoplot(diff)
...
```{r}
GlobalCountryTemp %>% filter(Country=="Global", year(Date) > 1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>% gg_tsdisplay(diff, plot_type = 'partial', lag_max = 15)
```

```{r}

fitting_land <- GlobalCountryTemp %>% filter(Country == "Global", year(Date)>1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>%
  model(
        ar3 = ARIMA(diff ~ pdq(3, 0, 0) + PDQ(0,0,0)),
        ar2 = ARIMA(diff ~ pdq(2, 0, 0) + PDQ(0,0,0)), 
        arma11 = ARIMA(diff ~ pdq(1, 1, 1) + PDQ(0,0,0)), 
        auto1 = ARIMA(diff)
        )
fitting_land %>% tidy()
...
```
```{r}
fit_land <- GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff = difference(LandAvgTemp, 12)) %>%  model(arima = ARIMA(diff ~ pdq(3, 0, 0)+ PDQ(0,0,0)))

fit_land %>% gg_tsresiduals(lag_max=15)
augment(fit_land) %>%
  features(.resid, ljung_box, lag = 10, dof = 3)
```
```{r}
fit_land %>%  forecast(h="8 years") %>% autoplot() #%>% (
  #GlobalCountryTemp %>% filter(Country == "Global", year(Date)>2000) %>% mutate(diff = difference(LandAvgTemp, 12)) %>% autoplot()
# )
  
```

Now for Land and Ocean
```{r}
GlobalCountryTemp %>%  filter(Country=="Global",  year(Date) > 1899) %>% 
autoplot(LandOceanAvgTemp)

```
```{r}
GlobalCountryTemp %>%  filter(Country=="Global",  year(Date) > 1899) %>% autoplot(LandOceanAvgTemp %>% difference(12))
```
...
```{r}
GlobalCountryTemp %>% filter(Country=="Global", year(Date) > 1899) %>% mutate(diff = difference(LandOceanAvgTemp, 12)) %>% gg_tsdisplay(diff, plot_type = 'partial', lag_max = 15)
```

```{r}
fitting_land_ocean <- GlobalCountryTemp %>% filter(Country == "Global", year(Date) > 1899) %>% mutate(diff = difference(LandOceanAvgTemp, 12)) %>%
  model(
        ar3 = ARIMA(diff ~ pdq(3, 0, 0) + PDQ(0,0,0)),
        ar2 = ARIMA(diff ~ pdq(2, 0, 0) + PDQ(0,0,0)), 
        arma11 = ARIMA(diff ~ pdq(1, 1, 1) + PDQ(0,0,0)), 
...
```{r}
fit_land_ocean <- GlobalCountryTemp %>% filter(Country=="Global",  year(Date) > 1899) %>% mutate(diff = difference(LandOceanAvgTemp, 12)) %>%  model(arima = ARIMA(diff ~ pdq(3, 0, 0)+ PDQ(0,0,0)))

fit_land_ocean %>% gg_tsresiduals(lag_max=15)
augment(fit_land_ocean) %>%
